...
function IsiRak() {
    ...
    const checkItem = () => {
        Axios.get("http://192.168.100.173:3333/rakterpakai", { headers: authHeader() })
            .then((res) => {
                setIsiRaks(res.data)
            }).catch(err => {
    ...
    const handleKeluar = (id) => {
        Axios.get(`http://192.168.100.173:3333/rakterpakai/${id}`, { headers: authHeader() })
            .then(res => {
                ...
                if (window.confirm("Keluarkan Barang?")) {
                    ...
                    Axios.delete(`http://192.168.100.173:3333/rakterpakai/${id}`, { headers: authHeader() }) //pake bactrik kalo mau ngirim parameter
                        .then(response => {
                            window.location.reload();
                        }).catch(err => {
                            if (err.response.status === 401) {
        ...
        <div className="container mt-4">
            ...
            <div className="row ">
                <div className="col-md-8 offset-md-2 text-center card-lain">
                    <h2><b> Data barang yang ada di rak</b></h2>
                </div>
            </div>
            ...
            <div className="container card-lain">
                <div className="row pb-3">
                    <div className="col-md-6 offset-md-3 rowSearch">
                        <input className="form-control search" type="text" value={search} onChange={e => setInput(e.target.value)} placeholder="Cari Nama Rak atau Produk" />
                    </div>
                </div>
                    ...
                    <tbody>
                        {
                            ...
                            itmeRaks.filter(isiRak => {
                                ...
                                if (isiRak.rak.nama.toLowerCase().includes(search) || isiRak.barang.produk.toLowerCase().includes(search)
                                    || isiRak.rak.nama.includes(search) || isiRak.barang.produk.includes(search)) {
                                    return true;
                                }
                                return false;
                            })
