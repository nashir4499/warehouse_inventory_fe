...
function IsiRak() {
    ...
    const handleKeluar = (id) => {
        Axios.get(`http://127.0.0.1:3333/rakterpakai/${id}`, { headers: authHeader() })
            .then(res => {
                ...
                if (window.confirm("Keluarkan Barang?")) {
                    ...
                    Axios.delete(`http://127.0.0.1:3333/rakterpakai/${id}`, { headers: authHeader() }) //pake bactrik kalo mau ngirim parameter
                        .then(response => {
                            window.location.reload();
                        }).catch(err => {
                            if (err.response.status === 401) {
